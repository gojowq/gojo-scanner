        ...
        if ($opcaoscanner == "0") {
            ...
            $logcatTime = shell_exec("adb logcat -d -v time | head -n 2");
                ...
                if (!empty($matchTime::1::)) {
                    ...
                    echo $bold . $roxo . "[+] Primeira log do sistema: " . $formattedDate . "\n";
            ...
            if ($fusoHorario !== "America/Sao_Paulo") {
                echo $bold . $roxo . "[!] Aviso: O fuso horário do dispositivo é '$fusoHorario', diferente de 'America/Sao_Paulo', possivel tentativa de Bypass.\n\n";
            }
            ...
            if (!empty($logsAlterados)) {
                ...
                foreach ($logsAlterados as $log) {
                    echo $bold . $roxo . "[!] Alterou horário de {$log['dataAntiga']} para {$log['dataNova']} {$log['horaNovaFormatada']} ({$log['acao']} horário)\n";
                }
            } else {
            ...
            if (!is_null($saida)) {
                ...
                foreach ($linhas as $linha) {
                    if (!empty($linha) && preg_match('/^([0-9]{2}-[0-9]{2}) ([0-9]{2}:[0-9]{2}:[0-9]{2}) (.+)$/', $linha, $matches)) {
                        ...
                        echo $bold . $roxo . "[!] " . $data . " " . $hora . " " . $branco . "$conteudo" . "\n";
                    }
                }
            } else {
                echo $bold . "\e[31m[!] Nenhum dado encontrado.\n";
            ...
            echo $bold . $azul . "[+] Checando se o replay foi passado...\n";
                ...
                if (!empty($resultadoPasta)) {
                    ...
                    if (!empty($matchAccessPasta::1::)) {
                        ...
                        echo $bold . $roxo . "[+] Data de acesso da pasta MReplays: $dataFormatada\n";
                        echo $bold . $roxo . "[*] Data de instalação do Free Fire: $dataInstalacaoFormatada\n";
                ...
                if ($arquivoBinMaisRecente !== null && $arquivoBinMaisRecente !== '') {
                    ...
                    if ($matchAccessBin) {
                        ...
                        echo $bold . $roxo . "[!] Não foi possível obter a data do último .bin.\n";
                    }
                } else {
                ...
                if (!empty($resultadoShaders)) {
                        ...
                        if (!empty($resultadoStat) && strpos($resultadoStat, "No such file or directory") === false) {
                            ...
                            if (!empty($matchModify::1::) && !empty($matchChange::1::) && !empty($matchAccess::1::)) {
                                ...
                                if ($diffInSeconds <= 3600) {
                                    echo $bold . $roxo . "[!] Possível bypass detectado: arquivo shader alterado recentemente.\n";
                                    echo $bold . $roxo . "[!] Arquivo: $nomeArquivo\n";
                                    echo $bold . $roxo . "[*] Hora da modificação: $dataModify\n";
                                    echo $bold . $roxo . "[*] Hora atual: " . $currentDateTime->format('d-m-Y H:i:s') . "\n\n";
                                ...
                                if ($dataModify === $matchChange::1:: && $dataModify === $matchAccess::1::) {
                                    if (stripos($nomeArquivo, 'shader') !== false) {
                                        if ($dataModify !== $dataInstalacao) {
                                            echo $bold . $roxo . "[!] Arquivo shader modificado: " . $nomeArquivo . "\n";
                                            echo $bold . $roxo . "[!] Horário da modificação: " . $dataModify . "\n"; 
                                            echo $bold . $roxo . "[*] Data de instalação do Free Fire: " . $dataInstalacaoFormatada . "\n";
                    ...
                    if ($encontrouBypass) {
                        echo $bold . $roxo . "[!] Modificação em arquivo de shaders detectada. Arquivo: " . $arquivoSuspeito . "\n";
                        echo $bold . $roxo . "[*] Hora da modificação: " . $dataModify . "\n";
                        echo $bold . $roxo . "[*] Verifique se a modificação ocorreu após a partida!\n\n";
                    }
                ...
                echo "\n" . $bold . $roxo . "[*] Data da última alteração na pasta 'gameassetbundles': " . ($dataChangeFormatada ?: "Não encontrada") . "\n";
                ...
                if ($resultadoDataModify !== '') {
                    try {
                        ...
                        echo $bold . $roxo . "[*] Data de modificação na pasta '$nomePasta': " . $dataModificacao->format('d-m-Y H:i:s') . "\n";
                ...
                if (!empty($resultadoObb)) {
                    ...
                    foreach ($arquivosObb as $arquivo) {
                        ...
                        if (!empty($resultadoDataChange)) {
                            ...
                            echo $roxo . "[*] Data de modificação do arquivo OBB: " . $dataChange->format("d-m-Y H:i:s") . "\n";
                        } else {
            ...
            echo $bold . $azul . "[+] Checando se o replay foi passado...\n";
                ...
                echo $bold . $branco . "\n\n\t Obrigadoo limpo de cheats.\n";
                echo $bold . $branco . "\t                 , EDUZADA...\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n";
